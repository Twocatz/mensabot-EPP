...
public class AveragePricePerDayAnalyzer<T> implements Analyzer<T> {
    @Override
    public OptionalWithMessage<T> analyze_unsafe(List<Menu> data) {
        ...
        for (Menu menu: data) {
            String date = convertDate(menu.getDate().toString());
            sb.append(date).append(": ");
            for (Meal meal: menu.getMeals()) {
                sum += meal.getPrice();
                size = menu.getMeals().size();
            }
            int avg = (sum/size);
            String result = euroFormat(avg);
    ...
    public String toString() {
        return "AveragePriceAnalyzerPerDay";
    }
}
